...
def compressWithRange(row):
    ...
    for item in ast.literal_eval(row[2]):
        ...
        oldStatus = item[0]
    if(sys.platform == 'linux'): csv_write = open('result/contractMethodTxStatusRange_timestamp.csv','a',newline = '')
    if(sys.platform == 'win32'): csv_write = open('result\\contractMethodTxStatusRange_timestamp.csv','a',newline = '')
...
if __name__ == '__main__':
    ...
    csv_file = open('result\\contractMethodTxStatus_timestamp.csv', 'r')
